...
let nodear = document.getElementById("nodear") as HTMLFormElement;
let aristear = document.getElementById("aristear") as HTMLFormElement;
...
nodear.addEventListener("submit", (event) => {
    ...
    let dato = info.get('nodo') as string;
    if (nodos.includes(dato)) return;
    let lista = document.getElementById("nodos") as HTMLUListElement;
    ...
    for (let i = 0; i < nodos.length; i++) inicial.push(0);
    matriz.push(inicial);
    console.log(matriz);
...
aristear.addEventListener("submit", (event) => {
    ...
    let primer = info.get('primer') as string;
    let segundo = info.get('segundo') as string;
    let arista = parseFloat(info.get('arista') as string);
    
    if (!nodos.includes(primer) || !nodos.includes(segundo)) return;
